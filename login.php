...
f.total,f.subtotal,f.recargo,f.descuento,f.pagado
from folio as f
...
union 
select df.cantidad as cantidad,df.descripcion,df.precio,f.total,f.subtotal,f.recargo,f.descuento,f.pagado
from  folio as f
...
inner join ciclo_escolar as ci on ci.ID_CICLO=dg.ID_CICLO
where f.id_folio=$ID_FOLIO";
//http://localhost/android/login.php?ID_FOLIO=76&opcion=4
	$rs=mysqli_query($con,$sql);
	   $i=0;
	   while ($row=mysqli_fetch_array($rs)) {	

	   	$datos[$i]=array("cantidad"=>$row[0],"descripcion"=>$row[1],"precio"=>$row[2],"total"=>$row[3],"subtotal"=>$row[4],
	   		"recargo"=>$row[5],"descuento"=>$row[6],"pagado"=>$row[7]);
	   		$i=$i+1; 
	   	}
...
inner join ciclo_escolar as ci on ci.ID_CICLO=dg.ID_CICLO
where u.ID_USUARIO=$ID_USUARIO ";
//http://localhost/android/login.php?ID_USUARIO=3&opcion=5
...
inner join ciclo_escolar as ci on ci.ID_CICLO=dg.ID_CICLO
where f.id_folio=$ID_FOLIO";
//http://localhost/android/login.php?ID_FOLIO=76&opcion=6
$rs=mysqli_query($con,$sql);
	   $i=0;
	   while ($row=mysqli_fetch_array($rs)) {	
